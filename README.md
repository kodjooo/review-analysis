...
- Для `yandex` добавлен circuit breaker: после серии подряд captcha-ошибок сервис временно прекращает новые yandex-запросы и переносит такие точки в последующие догоны.
