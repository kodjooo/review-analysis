...
from app.adapters.anti_bot import AntiBotDetectedError
...
class ReviewFetcher:
    ...
    def fetch_point_reviews(
        ...
        try:
            ...
            failure_kind = self.classify_error(error)
    ...
    def classify_error(error: Exception) -> FailureKind:
        if isinstance(error, AntiBotDetectedError):
            return FailureKind.ANTIBOT
        if isinstance(error, (TimeoutError, socket.timeout, socket.gaierror, URLError, ConnectionError)):
            return FailureKind.NETWORK
        if isinstance(error, ValueError):
            return FailureKind.PARSE

        message = str(error).lower()
        if any(marker in message for marker in ("captcha", "showcaptcha", "antibot", "антибот")):
            return FailureKind.ANTIBOT
        if any(
            marker in message
            for marker in (
            ...
            return FailureKind.NETWORK
        if any(marker in message for marker in ("не удалось извлечь", "parse", "selector", "invalid")):
            return FailureKind.PARSE
