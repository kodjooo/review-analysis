...
class YandexAdapter(BaseReviewAdapter):
    ...
    def parse_html(self, html: str) -> tuple[int, float, list[dict[str, Any]]]:
        ...
        review_count = self._extract_integer(
            ...
            soup.select_one(".business-summary-rating-badge-view__rating-count"),
        )
            ...
            reviews.append(
                {
                    ...
                    "author_name": self._text(node.select_one("[class*=review-author]")),
                    "published_at": self._text(node.select_one("time")),
                    "text": self._text(node.select_one("[class*=review-comment]")),
